...
// static inline float uniform_rnd_v()
// {
...
{
	fprintf(stderr, "Usage: %s model.ply data.ply [variant] [ntrials]\n", myname);
	exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	// Since we would like the x-axis error values grows exponentially from 0.01 to 1
	// we set factor to (0.01)^(1/16) [NUM_DATAPOINTS (in the figure) = 17]
	// and then each data point corresponds to (0.01)^((x+1)/16)
	...
	string filename = name1 + "_" + name2 + "_" +
		to_string(variant) + "_" + to_string(ntrials) + ".txt";
