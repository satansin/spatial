...
for rs in "${rs_eid[@]}"; do
	...
	DB_F="$DIR_OBJ_DB"/"${rs_range[rs]}"_rm/
	CURR_DB="$CURR_DS"/"${rs_range[rs]}"_rm
	mkdir -p "$CURR_DB"

	for qi in "${qi_range[@]}"; do

		for i_rm in "${rm_range[@]}"; do

			for nl in "${nl_range[@]}"; do

				Q_PLY="$DIR_OBJ_QUERY"/spec_rm/q_"$qi$i_rm"."$nl".ply
				CURR_NL="$CURR_DB"/q_"$qi$i_rm"_"$nl"
				mkdir -p "$CURR_NL"
				...
				for al_id in "${pcr_adv_eid[@]}"; do
					...
					if [ $al_id -eq 0 ] || [ $al_id -eq 1 ] || [ $al_id -eq 2 ] || [ $al_id -eq 3 ]; then
						...
						## donut / gt
						./"$OUT"/query_"${pcr_adv_alg[al_id]}".out "$DB_F" "$DIR_OBJ_INDEX"/object_scans_"${pcr_adv_idx[al_id]}"/"${rs_range[rs]}"_rm/"${rs_range[rs]}"_rm."$r".grid "$Q_PLY" "$e" -small \
							-stat="$CURR_AL"/"${pcr_adv_alg[al_id]}"."$r"."$e".dat

					elif [ $al_id -eq 4 ] || [ $al_id -eq 5 ]; then
					
						## 3nn
						./"$OUT"/query_"${pcr_adv_alg[al_id]}".out "$DB_F" "$DIR_OBJ_INDEX"/object_scans_"${pcr_adv_idx[al_id]}"/"${rs_range[rs]}"_rm/"${rs_range[rs]}"_rm."$r"."$a".grid "$Q_PLY" "$e" -small \
						...
						for m_id in "${m_eid[@]}"; do
							./"$OUT"/query_"${pcr_adv_alg[al_id]}".out "$DB_F" "$DIR_OBJ_INDEX"/object_scans_"${pcr_adv_idx[al_id]}"/"${rs_range[rs]}"_rm/"${rs_range[rs]}"_rm."$r".grid "$Q_PLY" "${m_range[m_id]}" -small \
						...
						## top-k: donut / gt (det)
						./"$OUT"/"${pcr_adv_alg[al_id]}".out "$DB_F" "$DIR_OBJ_INDEX"/object_scans_"${pcr_adv_idx[al_id]}"/"${rs_range[rs]}"_rm/"${rs_range[rs]}"_rm."$r".grid "$Q_PLY" -small \
						...
						## top-k: donut / gt (prob)
						./"$OUT"/"${pcr_adv_alg[al_id]}".out "$DB_F" "$DIR_OBJ_INDEX"/object_scans_"${pcr_adv_idx[al_id]}"/"${rs_range[rs]}"_rm/"${rs_range[rs]}"_rm."$r".grid "$Q_PLY" "$e" -small \
						-stat="$CURR_AL"/"${pcr_adv_alg[al_id]}"."$r"."$e".dat

					elif [ $al_id -eq 27 ]; then

						## top-k: 3nn (prob)
						./"$OUT"/"${pcr_adv_alg[al_id]}".out "$DB_F" "$DIR_OBJ_INDEX"/object_scans_"${pcr_adv_idx[al_id]}"/"${rs_range[rs]}"_rm/"${rs_range[rs]}"_rm."$r"."$a".grid "$Q_PLY" "$e" -small \
						-stat="$CURR_AL"/"${pcr_adv_alg[al_id]}"."$r"."$a"."$e".dat
